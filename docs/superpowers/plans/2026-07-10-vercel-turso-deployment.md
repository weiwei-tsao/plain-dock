...
**Tech Stack:** Next.js 16, Prisma 6.19, SQLite, Turso/libSQL, `@prisma/adapter-libsql`, `@libsql/client`, TypeScript strict.
...
- Modify: `package.json`
  - Add `db:deploy` script.
  - Add Turso/libSQL runtime dependencies through `npm install`.
- Modify: `package-lock.json`
  - Updated by `npm install @prisma/adapter-libsql @libsql/client`.
...
- Turso migrations remain an operator command: `DATABASE_URL=... TURSO_AUTH_TOKEN=... npm run db:deploy`.
- There is no test runner configured in this repo. Verification is static checks, build, and manual runtime checks.

---

### Task 1: Add Turso Dependencies And Migration Script

**Files:**
- Modify: `package.json`
- Modify: `package-lock.json`

- [ ] **Step 1: Install libSQL dependencies**
...
Expected:
- `package.json` gains entries for `@prisma/adapter-libsql` and `@libsql/client` under `dependencies`.
...
- [ ] **Step 2: Add the Prisma deploy script**
...
```json
  "scripts": {
    ...
    "remotedev": "next dev --turbopack -p 3000 --hostname 0.0.0.0"
  },
```
...
- [ ] **Step 3: Verify dependency install and script shape**
...
- No code imports the new dependencies yet, so this verifies the install did not break generated Prisma types.

- [ ] **Step 4: Commit**

```bash
git add package.json package-lock.json
git commit -m "chore(db): add turso prisma dependencies"
```
...
- [ ] **Step 5: Update Scripts table**

In `README.md`, add this row to the scripts table after `npm run typecheck`:

```md
| `npm run db:deploy` | Apply Prisma migrations to the configured database |
```

- [ ] **Step 6: Update Architecture server library description**
...
如果要迁移已有的本地 SQLite 数据，请先备份数据库文件，再用 Turso CLI 工具导入数据，确认远程 `Note` 记录无误后，再对 Turso 执行或对齐 Prisma 迁移。不要依赖 Vercel 的本地文件系统保存笔记。
````

- [ ] **Step 5: Update Chinese scripts table**

In `README.zh.md`, add this row after the `npm run typecheck` row:

```md
| `npm run db:deploy` | 对当前配置的数据库执行 Prisma 迁移 |
```

- [ ] **Step 6: Update Chinese architecture db line**
...
- [ ] **Step 5: Verify real Turso path if credentials are available**

If the user provides a real Turso database URL and token, run:
...
Expected:
- `npm run db:deploy` applies migrations or reports that there are no pending migrations.
