...
3. 从本地手动执行 Prisma 迁移：
   ...
   以后每次新增 Prisma migration 后，都要重新执行这个命令。

4. 部署到 Vercel。

如果要迁移已有的本地 SQLite 数据，请先备份数据库文件，再用 Turso CLI 工具导入数据，确认远程 `Note` 记录无误后，如果迁移历史还没有写入 Turso，再对 Turso 执行 `npm run db:deploy`。不要依赖 Vercel 的本地文件系统保存笔记。
